...
public class Robot {
    ...
    public int countFriendlyHQs() throws GameActionException{
        int count = 0;
        for(int i = FRIENDLY_HQ_MIN_INDEX; i <= FRIENDLY_HQ_MAX_INDEX; i++){
    ...
    public void fuzzyMove(Direction dir) throws GameActionException{
        ...
        for(Direction d : fuzzyDirs){
            MapLocation locInDir = rc.getLocation().add(d);
            if(!pushedAway(locInDir) && tryMove(d)){
                return;
            }
        }
    }
...
//        rc.setIndicatorDot(loc,0,255,255);
        ...
        for(Direction d : fuzzyDirs){
            MapLocation locInDir = rc.getLocation().add(d);
            if(!pushedAway(locInDir, loc) && loc.distanceSquaredTo(locInDir)<minDist && rc.canMove(d)){
                rc.move(d);
                minDist = distTo(loc);
                return;
            }
        ...
        for (int i = 0; i < 9; i++){
            // follow wall
            if(!pushedAway(rc.getLocation().add(d), loc) && foundWall && rc.canMove(d)){
                tryMove(d);
                return;
            }
            // find wall
    ...
    public boolean pushedAway(MapLocation moveLoc, MapLocation targetLoc) throws GameActionException {
        return rc.onTheMap(moveLoc) && (rc.senseMapInfo(moveLoc).getCurrentDirection().equals(moveLoc.directionTo(targetLoc).opposite()) ||
                rc.senseMapInfo(moveLoc).getCurrentDirection().equals(rc.getLocation().directionTo(moveLoc).opposite()));
    }

    public boolean pushedAway(MapLocation moveLoc) throws GameActionException {
        return rc.onTheMap(moveLoc) && rc.senseMapInfo(moveLoc).getCurrentDirection().equals(rc.getLocation().directionTo(moveLoc).opposite());
    }
